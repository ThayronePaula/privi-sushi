...
export const GENERAL = (i18n: I18n): StrategyGeneralInfo => ({
  ...
  description: i18n._(
    t`Stake SUSHI for xSUSHI into Cream and deposit crXSUSHI into BentoBox in one click.`
  ),
...
const useStakeSushiToCreamToBentoStrategy = (): StrategyHook => {
  ...
  useEffect(() => {
    ...
    setBalances({
      ...
      outputTokenBalance: tryParseAmount(
        crxSushiBentoBalance?.value?.toFixed(8) || "0",
        CRXSUSHI
      ),
    });
  ...
  const calculateOutputFromInput = useCallback(
    async (
      ...
      if (zapIn) {
        ...
        const cValue = await zenkoContract.fromCtoken(
          CRXSUSHI.address,
          inputValue.toBigNumber(inputToken.decimals)
        );
        const value = BigNumber.from(cValue).mulDiv(
          sushiPerXSushi.toString().toBigNumber(18),
          e10(18)
        );
