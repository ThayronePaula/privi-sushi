...
import { buildEtherscanApiUrl, prepareTransactions } from "../helpers";
...
const useEtherscan = (ethAddress) => {
  ...
  const getWalletBalance = async () => {
    try {
      const response = await fetch(buildEtherscanApiUrl(ethAddress, "balance"));
  ...
  const getNormalTransactions = async () => {
    try {
      ...
      const json = await response.json();
      !!json.result.length &&
        setNormalTransactions(prepareTransactions(json.result));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getInternalEthTransactions = async () => {
    try {
      ...
      const json = await response.json();
      !!json.result.length &&
        setIntenalTransactions(prepareTransactions(json.result));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getTokenTransactions = async () => {
    try {
      const response = await fetch(buildEtherscanApiUrl(ethAddress, "tokentx"));

      const json = await response.json();
      !!json.result.length &&
        setTokenTransactions(prepareTransactions(json.result));
    } catch (error) {
      console.log(error);
    }
  };
