...
const InariButton: FC<InariButtonProps> = ({ children, ...rest }) => {
  ...
  const { balances, execute, bentoApproveCallback, approveCallback } =
    useSelectedInariStrategy();
  ...
  const steps = [];
  if (approveCallback)
    steps.push(approveCallback[0] === ApprovalState.APPROVED);
  if (bentoApproveCallback)
    steps.push(
      bentoApproveCallback.approvalState === BentoApprovalState.APPROVED
    );
  ...
  if (
    ...
    balances.inputTokenBalance.lessThan(inputValue)
  )
    return (
      <Button {...rest} disabled color="gray">
    ...
    return (
      <>
        <Button {...rest} disabled color="gray">
          <Dots>
            {i18n._(
              t`Approving Inari to spend ${approveCallback[2].currency.symbol}`
            )}
          </Dots>
        </Button>
        {approveFlow}
    ...
    return (
      <>
        <Button {...rest} color="pink" onClick={approveCallback[1]}>
          {i18n._(
            t`Approve Inari to spend ${approveCallback[2].currency.symbol}`
          )}
        </Button>
        {approveFlow}
      </>
  ...
  if (
    bentoApproveCallback &&
    bentoApproveCallback.approvalState === BentoApprovalState.PENDING
  )
    return (
      <>
        <Button {...rest} disabled color="gray">
  ...
  if (
    bentoApproveCallback &&
    bentoApproveCallback.approvalState === BentoApprovalState.NOT_APPROVED
  )
    return (
      <>
