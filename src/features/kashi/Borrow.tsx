...
export default function Borrow({ pair }: BorrowProps) {
  ...
  const allowedSlippage = useUserSlippageToleranceWithDefault(
    DEFAULT_BORROW_SLIPPAGE_TOLERANCE
  ); // custom from users
  ...
  const extraCollateral =
    swap && foundTrade
      ? BigNumber.from(
          foundTrade.minimumAmountOut(allowedSlippage).quotient.toString()
        )
      : ZERO;
  ...
  const swapCollateral = collateralValue.toBigNumber(
  ...
  const maxBorrow = nextMaxBorrowPossible.toFixed(
