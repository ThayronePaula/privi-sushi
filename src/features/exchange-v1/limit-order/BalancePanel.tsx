...
const BalancePanel: FC = () => {
  ...
  const { walletBalances, bentoboxBalances, currencies } =
    useDerivedLimitOrderInfo();
    ...
    <div className="grid grid-cols-2 bg-dark-700 rounded-b px-5 py-1">
      <div className="flex gap-2">
        ...
        <Typography
          ...
          onClick={() => handleMaxInput(true)}
        >
          {bentoboxBalances[Field.INPUT]?.toSignificant(6, {
            groupSeparator: ",",
          })}{" "}
      ...
      <div className="flex gap-2">
        ...
        <Typography
          variant="sm"
          className="text-secondary"
          onClick={() => handleMaxInput(false)}
        >
          {walletBalances[Field.INPUT]?.toSignificant(6, {
            groupSeparator: ",",
          })}{" "}
          {currencies[Field.INPUT]?.symbol}
        </Typography>
