    ...
    <div>
      <div className="inline-flex items-center space-x-4 flex-inline">
        ...
        <div className="flex flex-col space-y-1">
          {rewards?.map((reward, i) => {
            const decimals =
              6 - String(reward?.rewardPerDay?.toFixed(0)).length;
            return (
              <div key={i} className="text-base whitespace-nowrap">
                {reward?.rewardPerDay?.toFixed(decimals > 0 ? decimals : 0)}{" "}
                {reward.token}
              </div>
            );
          })}
      ...
      },
      {
        ...
        Cell: (props) => (
          <div className="inline-flex flex-row font-medium">
            ...
            &nbsp;/{" "}
            {props.value.monthly > 10000
              ? ">10,000%"
              : formatPercent(props.value.monthly * 100)}
            &nbsp;/{" "}
            {props.value.daily > 10000
              ? ">10,000%"
              : formatPercent(props.value.daily * 100)}
          </div>
        ),
        align: "right",
      ...
      },
      {
        ...
        Cell: (props) => (
          <div className="text-base font-medium text-primary">
            {formatNumber(props.value, true, false)}
          </div>
        ),
        align: "right",
      },
