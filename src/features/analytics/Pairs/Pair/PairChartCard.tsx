  ...
  liquidity: {
    ...
    getData: (pair, pair1d, pair2d, dayData) => ({
      ...
      chart: dayData
        ...
        .map((day, i) => ({ x: i, y: Number(day.liquidityUSD) })),
    }),
  },
  volume: {
    header: "Volume",
    getData: (pair, pair1d, pair2d, dayData) => ({
      ...
      chart: dayData
        ?.sort((a, b) => a.date - b.date)
        .map((day, i) => ({ x: i, y: Number(day.volumeUSD) })),
    }),
  },
};
...
export default function PairChartCard(props: PairChartCardProps): JSX.Element {
  ...
  const pair = useSushiPairs({
    subset: [props.pair],
    shouldFetch: !!props.pair,
    chainId,
  })?.[0];
  ...
  const data = useMemo(
    () => type.getData(pair, pair1d, pair2d, dayData),
    [pair, pair1d, pair2d, dayData]
  );
