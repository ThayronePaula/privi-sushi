      ...
      ? pairsFiltered.map((pair) => {
          ...
          return {
            ...
            userBalanceUSD:
              ...
              userPair.liquidityTokenBalance,
        ...
        ? userPositions
            .map((position) => {
              const farm = farms.find(
                (f) => f.chef === position.chef && f.id === position.id
              );
              if (!farm) return undefined;
              const sushiPair = sushiPairs?.find(
                (pair) => pair.id === farm.pair
              );
              const kashiPair = kashiPairs?.find(
                (pair) => pair.id === farm.pair
              );
              ...
              return {
                ...
                userStakedUSD: sushiPair
                  ? ((sushiPair.reserveUSD / sushiPair.totalSupply) *
                      position.amount) /
                    1e18
                  : (position.amount /
                      getFraction(kashiPair) /
                      10 ** kashiPair.token0.decimals) *
    ...
    return bentoUserTokens && tokens && nativePrice
      ? bentoUserTokens.map((bentoToken) => {
          ...
          return {
            ...bentoToken,
            valueUSD:
              (bentoToken.amount * token?.derivedETH * nativePrice) /
                10 ** token?.decimals ?? 0,
            chain: ChainId[chainId],
          };
        })
...
export function useUserKashiPairs(chainId = undefined) {
  ...
  const tokens = useTokens({
    subset: kashiTokenAddresses,
    chainId,
    shouldFetch: !!kashiTokenAddresses,
  });
  ...
  return useMemo(() => {
    return userKashiPairs && tokens && nativePrice
      ? userKashiPairs.map((userPair) => {
          ...
          const collateral = tokens.find(
            (t) => t.id === userPair.pair.collateral.id
          );

          const assetValueUSD =
            (userPair.assetAmount / 10 ** asset.decimals) *
            asset.derivedETH *
            nativePrice;
          const borrowedValueUSD =
            (userPair.borrowedAmount / 10 ** asset.decimals) *
            asset.derivedETH *
            nativePrice;
          const collateralValueUSD =
            (userPair.collateralAmount / 10 ** collateral.decimals) *
            collateral.derivedETH *
            nativePrice;
