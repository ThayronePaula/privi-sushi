...
const MeowshiButton: FC<MeowshiButtonProps> = ({ meowshiState }) => {
  ...
  const { approvalState, approve, meow, unmeow, meowSushi, unmeowSushi } =
    useMeowshi(currencies[Field.INPUT] === SUSHI[ChainId.MAINNET]);
    ...
    if (doMeow) {
      if (currencies[Field.INPUT]?.symbol === "SUSHI") {
        tx = await meowSushi({
          value: parseUnits(
            fields[Field.INPUT],
            sushiBalance.currency.decimals
          ),
  ...
  const buttonDisabledText = useMemo(() => {
    ...
    if (parsedInputAmount?.greaterThan(balance))
      return i18n._(t`Insufficient Balance`);
    if (!parsedInputAmount?.greaterThan(0))
      return i18n._(t`Please enter an amount`);
    return null;
    ...
    return (
      <>
        <TransactionConfirmationModal
          ...
          pendingText={i18n._(
            t`Converting ${parsedInputAmount?.toSignificant(6, {
              groupSeparator: ",",
            })} ${
              meowshiState.currencies[Field.INPUT]?.symbol
            } for ${parsedOutputAmount?.toSignificant(6, {
              groupSeparator: ",",
